...
View(fang)
```
### View can also give you an idea of the structure of the data. Here, we can see headers, and the data included in each vertor. 
```{r}
table(fang$Group)
```
...
View (snp)
```
### Again, this allows us to further familiarize ourselves with the data.
...
snp_sub_vis <- snp_sub_vis %>% pivot_longer(starts_with("V"),
                             ...
                             values_to='SNP')
### plot
all_chromosomes_plot <- ggplot (data = snp_sub_vis) + geom_bar(mapping = aes(x = Chromosome, fill = Chromosome)) +xlab(label = "Chromosome") + ylab(label = "SNPs") + ggtitle("Single Nucleotide Polymorphism per Chromosome") + scale_x_discrete(limit=c(1:10))
```
### Chromosome 1 seems to have the highest count, while chromosome 10 has the lowest
...
### graph 1
by_sample_plot <- ggplot (data = Subsetted_SNPs) + geom_bar(mapping = aes(x = Chromosome, fill = type)) + 
xlab(label = "Chromosome") + ylab(label = "SNPs") +
  ggtitle("Single Nucleotide Polymorphism per Sample") 
### graph 2
by_sample_plot_group <- by_group_plot + facet_grid(Group ~ .)
```
...
homo_vs_hetero_group_sample <- ggplot(data = snp_vis_2_snp_na_omit) + 
...
homo_vs_hetero_group <- ggplot(data = snp_vis_2_snp_na_omit) + 
  geom_bar(mapping = aes(x = Group, fill = homozygosity)) + 
  xlab(label = "Homozygous vs. Heterozygous") + ylab(label = "Count") +
  ggtitle("Homozygous vs. Heterozygous by Group") 

homo_vs_hetero_sample <- ggplot(data = snp_vis_2_snp_na_omit) + 
...
missing_data_plot <- ggplot(data = r_is_hard) + 
...
missing_data_plot2 <- ggplot(data = r_is_hard) +
...
Amplicons_Sample <- ggplot(Subsetted_SNPs_new) + geom_col(aes(x = type, y = count_amplicons)) + xlab(label = "Sample") + ylab(label = "Number of Amplicons") + ggtitle("Amplicons per Sample")
