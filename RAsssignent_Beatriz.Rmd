...
### graph 1
ggplot (data = Subsetted_SNPs) + geom_bar(mapping = aes(x = Chromosome, fill = type)) + 
xlab(label = "Chromosome") + ylab(label = "SNPs") +
  ggtitle("Single Nucleotide Polymorphism per Sample") 

ggsave("Chromosome by Sample.png")

### graph 2
by_group_plot + facet_grid(Group ~ .)

ggsave("Chromosome by Group + Sample.png")
```
...
ggsave("hom vs. het groups _ sample.png")
...
ggsave("Missing Data Sample & Group.png")
...
ggsave("Missing Data by Sample.png")
```
